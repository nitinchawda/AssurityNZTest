...
This suite is pure API testing (Playwright `request` fixture); no browser is launched, so this step can be skipped.
...
| Command | Purpose |
|---------|---------|
| `npm test` | Run all tests (also writes the HTML report to `playwright-report/`) |
...
The HTML report is generated by `npm test` into `playwright-report/`. To open it:
